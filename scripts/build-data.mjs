// Builds site/data/streaks.json: the longest *active* hitting streaks in MLB.
//
// The official MLB /streaks endpoint now sits behind authentication, so we
// compute streaks ourselves from public, unauthenticated endpoints:
...
const DIMAGGIO = { holder: "Joe DiMaggio", length: 56, year: 1941, team: "New York Yankees" };
...
async function main() {
  ...
  console.log(`Building hitting-streak data for ${SEASON}...`);
  ...
  console.log(`Scanning ${hitters.length} position players...`);

  const computed = await mapLimit(hitters, CONCURRENCY, async (player) => {
    const games = await getGameLog(player.id);
  ...
  const leader = streaks[0];
  console.log(
    `Wrote ${streaks.length} streaks to ${OUT_FILE} in ${secs}s.` +
...
main().catch((err) => {
  ...
  process.exit(1); // fail the build loudly rather than ship stale data
});
